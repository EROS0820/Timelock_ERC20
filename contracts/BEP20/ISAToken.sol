...
contract ISAToken is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        _symbol = "ISAT";
        _decimals = 18;
        _mint(_msgSender(), 1000000000000 * (10 ** _decimals));
        emit Transfer(address(0), _msgSender(), 1000000000000 * (10 ** _decimals));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        for(uint256 i = 0; i < _buyHistories[sender].length; i++) {
            ...
            if (_history.timestamp < block.timestamp - 300) {
        ...
        for(uint256 i = 0; i < _buyHistories[sender].length; i++) {
            ...
            if (deleteAmount + _history.amount <= amount)
            {
                ...
                _balances[sender] = _balances[sender].sub(_history.amount, "BEP20: transfer amount exceeds balance");
                _balances[recipient] = _balances[recipient].add(_history.amount);
                delete _buyHistories[sender][i];
            }
            else if (deleteAmount < amount && deleteAmount + _history.amount > amount)
            {
                _balances[sender] = _balances[sender].sub((amount - deleteAmount), "BEP20: transfer amount exceeds balance");
                _balances[recipient] = _balances[recipient].add((amount - deleteAmount));
                _history.amount.sub((amount - deleteAmount));
                deleteAmount = amount;
                break;
            }
        }
